...
#include <jni.h>
#include "common.h"
#include "jni/eu_aschuetz_nativeutils_impl_JNILinuxNativeUtil.h"
#include "jni/eu_aschuetz_nativeutils_impl_NativeLibraryLoaderHelper.h"
#include <stddef.h>
#include <stdbool.h>
#include <stdlib.h>
#include <assert.h>
...
JNIEXPORT jlong JNICALL Java_eu_aschuetz_nativeutils_impl_NativeLibraryLoaderHelper_getNativeLibVersion
  (JNIEnv * env, jclass clazz) {
	return eu_aschuetz_nativeutils_impl_NativeLibraryLoaderHelper_EXPECTED_NATIVE_LIB_VERSION;
}
...
bool makeRefs(JNIEnv * env) {
	...
	JNINativeMemory = makeGlobalClassRef(env, "eu/aschuetz/nativeutils/impl/JNINativeMemory");
	if (JNINativeMemory == NULL) {
		(*env) -> ExceptionClear(env);
		(*env) -> ThrowNew(env, Exception, "cant find eu/aschuetz/nativeutils/impl/JNINativeMemory");
		return false;
	}
	...
	if (JNINativeMemory_ptr == NULL) {
		(*env) -> ExceptionClear(env);
		(*env) -> ThrowNew(env, Exception, "cant find eu/aschuetz/nativeutils/impl/JNINativeMemory.ptr");
		return false;
	}
	...
	badFDClass = makeGlobalClassRef(env, "eu/aschuetz/nativeutils/api/exceptions/InvalidFileDescriptorException");
	if (badFDClass == NULL) {
		(*env) -> ExceptionClear(env);
	...
	unknownErrorClass = makeGlobalClassRef(env, "eu/aschuetz/nativeutils/api/exceptions/UnknownNativeErrorException");
	if (unknownErrorClass == NULL) {
		(*env) -> ExceptionClear(env);
